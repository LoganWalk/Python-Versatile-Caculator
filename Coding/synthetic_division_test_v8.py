...
def main():
    ...
    if len(exp_list) > 1:
        for e in range(len(exp_list)):
            fill_in = int(exp_list[e-1] - exp_list[e])
            e += 1
            if(fill_in > 1):
    ...
    token_2 = tokens_2[2]
    if '-' in tokens_2[1]:
        divisor = float(token_2)
    elif '+' in tokens_2[1]:
        divisor = float(token_2) * -1
    if tokens_2[0] == 'x':
        divisor_exp = 'x'

    first_number = coeff_list[0]
    addition = first_number
    for number in range(len(coeff_list)-1):
        
        multiplier = divisor * addition
        addition = coeff_list[number + 1] + multiplier
